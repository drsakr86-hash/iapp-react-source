 ...
 *
 * STORAGE PROVIDERS
 * storage_provider is either 'supabase' (files live in the private
 * `medical-imaging` Supabase Storage bucket, same as before) or 'r2'
 * (files live in a private Cloudflare R2 bucket, added for extra imaging
 * storage headroom). 'cloudinary' and 'external' rows are legacy —
 * absolute URLs migrated in as-is, never written again (see signedUrl()).
 * R2 access goes through the `r2-presign` Supabase Edge Function, which
 * signs short-lived upload/read URLs and re-checks the caller can see the
 * patient before signing anything — the same access boundary the Supabase
 * Storage bucket policy enforces for 'supabase' rows.
 * ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * R2 presign. Calls the `r2-presign` Edge Function, which holds the R2
 * credentials — this file never sees them. The function re-validates the
 * path shape and the caller's access to the patient before signing, so a
 * short-lived URL is the only thing that ever crosses the wire either way.
 * ---------------------------------------------------------------------- */
async function r2Presign(
  ...
  const { data, error } = await supabase.functions.invoke<{
    ...
    ok?: boolean;
    error?: string;
  }>('r2-presign', { body: { action, path, ttl, contentType } });
  if (error) throw new Error(reportError(error, 'الوصول إلى مساحة R2'));
  if (data?.error) throw new Error(data.error);
  if (action === 'delete') return '';
  if (!data?.url) throw new Error('لم يتم استلام رابط من الخادم');
  return data.url;
}
...
/**
 * Soft-delete only: the row is marked deleted_at and disappears from every
 * list immediately, but the underlying file (Supabase Storage or R2) is
 * left in place. A medical image is not something to remove outright from
 * the recoverable side by mistake; if it must ever be purged for good,
 * that is a deliberate, separate action against the database and storage
 * directly — not a button in this screen.
 */
export async function deleteStudy(study: { id: string; patient_id: string }): Promise<void> {
  const { error } = await supabase
    .from('medical_images')
    .update({ deleted_at: new Date().toISOString() })
    .eq('id', study.id);
  if (error) throw new Error(reportError(error, 'حذف الصورة'));
  await audit('delete', study);
}
...
export async function signedUrl(
  ...
  if (study.storage_provider === 'r2') {
    /* R2 presigned GETs don't carry Supabase's download-filename option;
       downloads still work, just without a forced Content-Disposition. */
    const url = await r2Presign('read', path, opts.ttl ?? SIGNED_URL_TTL_SECONDS);
...
export async function thumbUrls(
  ...
  if (r2Items.length) {
    /* R2 has no batch-signing endpoint here, so these go one at a time;
       thumbnails are decoration, so a failure on one is silently skipped. */
    await Promise.all(
      r2Items.map(async (s) => {
        try {
          out[s.id] = await signedUrl(s, { thumb: true, ttl, silent: true });
        } catch {
          /* decoration only */
        }
      }),
...
export interface UploadInput {
  ...
  notes?: string | null;
  /** Defaults to 'supabase' — pass 'r2' to store this study in R2 instead. */
...
export async function upload(input: UploadInput): Promise<Study> {
  ...
  try {
    ...
    if (provider === 'r2') {
      ...
      if (!put.ok) throw new Error(`فشل الرفع إلى مساحة R2 (${put.status})`);
    } else {
    ...
    if (uploaded.length) {
      try {
        if (provider === 'r2') {
          await r2Presign('delete', uploaded[0]);
        } else {
