...
COMMENT ON SCHEMA "iapp" IS 'مخطط العيادة السريري - I APP. قاعدة بيانات جديدة، لا علاقة لها بأي نظام سابق.';
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    -- الحذف الناعم يُسجَّل حذفاً لا تعديلاً
    v_action := case when new.deleted_at is not null and old.deleted_at is null
                     then 'delete' else 'update' end;
...
COMMENT ON COLUMN "iapp"."medical_images"."storage_path" IS 'مسار الكائن داخل الحاوية الخاصة medical-imaging. ليس رابطاً عاماً.';



COMMENT ON COLUMN "iapp"."medical_images"."legacy_url" IS 'روابط Cloudinary القديمة — للقراءة فقط ولا يُكتب فيها شيء جديد.';



COMMENT ON COLUMN "iapp"."medical_images"."thumbnail_path" IS 'مسار المصغّرة داخل نفس الحاوية الخاصة. تُقرأ بروابط موقّعة فقط.';
    ...
    AS $$
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                begin
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  if not (iapp.is_doctor() or public.is_admin()) then
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      raise exception 'not_authorized: طباعة الطلب للطبيب أو المدير';
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        end if;
...
CREATE OR REPLACE FUNCTION "iapp"."reschedule_appointment"("p_id" "uuid", "p_new_date" "date", "p_new_time" time without time zone, "p_reason" "text" DEFAULT 'إعادة جدولة'::"text", "p_new_doctor_id" "uuid" DEFAULT NULL::"uuid", "p_new_room" "text" DEFAULT NULL::"text") RETURNS "iapp"."appointments"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'iapp', 'public'
    AS $$
declare
...
begin
  ...
  return null;
exception when others then
  -- التدقيق لا يُسقط عملاً سريرياً: لو تعذّرت الكتابة (قسم مفقود مثلاً)
  -- يُرفع تحذير ويمضي الصفّ. الصمت التام هنا أسوأ من التحذير.
...
CREATE OR REPLACE FUNCTION "iapp"."tg_imaging_defaults"() RETURNS "trigger"
    ...
    AS $$
begin
  -- study_date هو التاريخ المعتمد، و captured_on يبقى مرآة له
  -- حتى لا تنكسر الشاشات التي ما زالت تقرأ العمود القديم.
  ...
  if new.study_date > current_date then
    raise exception 'imaging_future_date: تاريخ الدراسة في المستقبل';
  end if;

  if new.status = 'reported'::iapp.imaging_status
     and coalesce(btrim(new.doctor_report), '') = '' then
    raise exception 'imaging_report_required: لا يمكن اعتماد دراسة بلا تقرير';
  end if;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       || '-' || lpad(nextval('iapp.imaging_order_seq')::text, 5, '0');
                                                                                                                                                                                                                                                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                               if new.created_by is null then new.created_by := auth.uid(); end if;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   if new.ordered_on > current_date then
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       raise exception 'order_future_date: تاريخ الطلب في المستقبل';
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         end if;
...
COMMENT ON FUNCTION "iapp"."tg_reject_noop_transition"() IS 'المرحلة 8: يمنع التحديث الضائع حين ينفّذ شخصان نفس الإجراء في نفس اللحظة. يراقب أعمدة الفاعل لأن iapp.transition تكتبها بلا شرط على تغيّر الحالة. الاسم يبدأ بـ 00 ليُنفَّذ قبل مشغّل التحقق — ترتيب المشغّلات أبجدي.';
...
begin
  ...
  -- تحليل فاشل لا يُراجَع: لا يوجد ما يُقبل أو يُعدَّل.
...
begin
  if new.role is distinct from old.role then
    if auth.uid() is not null and not public.is_admin() then
      raise exception 'غير مسموح: المدير وحده يغيّر الأدوار';
    end if;
    if new.id = auth.uid() then
      raise exception 'غير مسموح: لا يمكن تغيير دورك بنفسك';
    end if;
  end if;
  new.updated_at := now();
  return new;
...
COMMENT ON TABLE "iapp"."exam_findings" IS 'موجودات القطاع الأمامي والخلفي، صف لكل (فحص، عين، حقل) — تدعم OD/OS/OU';



COMMENT ON COLUMN "iapp"."exam_findings"."value" IS 'نص حرّ. القوائم في الواجهة اقتراح لا حصر';
...
COMMENT ON COLUMN "iapp"."examinations"."va_right" IS 'UCVA OD — حدة الإبصار بلا تصحيح، نص لا رقم';
...
COMMENT ON COLUMN "iapp"."examinations"."va_right_corrected" IS 'BCVA OD — بأفضل تصحيح';
...
COMMENT ON COLUMN "iapp"."examinations"."va_right_ph" IS 'PH OD — بالثقب';



COMMENT ON COLUMN "iapp"."examinations"."va_left_ph" IS 'PH OS — بالثقب';
...
COMMENT ON COLUMN "iapp"."iop_measurements"."method" IS 'Goldmann · NCT · Tonopen · iCare · Perkins · digital — رقمان بجهازين مختلفين لا يُقارنان';
...
COMMENT ON COLUMN "iapp"."patients"."primary_condition" IS 'الحالة الرئيسية كما يكتبها الطبيب. التشخيصات المفصَّلة في جدول diagnoses.';



COMMENT ON COLUMN "iapp"."patients"."triage_status" IS 'تصنيف تشغيلي سريع: مكتمل / متابعة / طارئ.';



COMMENT ON COLUMN "iapp"."patients"."last_visit" IS 'يُحدَّث تلقائياً بمشغّل عند كل زيارة — لا يُدخل يدوياً.';
...
COMMENT ON TABLE "iapp"."surgeries" IS 'التاريخ الجراحي — أُجريت عندنا أو يرويها المريض (is_external)';
...
COMMENT ON VIEW "iapp"."v_exam_full" IS 'الفحص + ضغط العين + الانكسار في صف واحد — أساس مقارنة الزيارة بسابقتها';
